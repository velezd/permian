...
from libpipeline.plugins.beaker_tag import BeakerTagReportSender
...
class TestBeakerTagReportSender(unittest.TestCase):
    ...
    def test_dryrun_minimal(self):
        with self.assertLogs('libpipeline.plugins.beaker_tag', level='INFO') as cm:
        ...
        self.assertEqual(cm.output, ['INFO:libpipeline.plugins.beaker_tag:Setting tag ACCEPTED to compose RHEL2-20310314'])

    @patch('libpipeline.plugins.beaker_tag.xmlrpc_server')
    def test_minimal(self, xmlrpc_server_mock):
        bkr_mock = Mock()
        xmlrpc_server_mock.return_value = bkr_mock

        with self.assertLogs('libpipeline.plugins.beaker_tag', level='DEBUG') as cm:
            self.report_sender = BeakerTagReportSender(self.library.testplans['Beaker tag testplan 2'],
                                                       self.library.testplans['Beaker tag testplan 2'].reporting[0],
                                                       self.caseRunConfigurations,
                                                       DummyEvent1(),
        ...
        self.assertEqual(cm.output, ['DEBUG:libpipeline.plugins.beaker_tag:Successfully set tag ACCEPTED to compose RHEL2-20310314'])
    ...
    def test_dryrun_all(self):
        ...
        self.assertEqual(cm.output, ['INFO:libpipeline.plugins.beaker_tag:Setting tag NOT-FAILED-NIGHTLY to compose RHEL2-20310314'])
    ...
    def test_dryrun_no_report_1(self):
        ...
        self.assertEqual(cm.output, ['DEBUG:libpipeline.plugins.beaker_tag:Not reporting: condition or report-on-results did not match'])
